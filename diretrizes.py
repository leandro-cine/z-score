...
def classificar_peso_ig(peso_g: float, ig_sem: float):
    ...
    if ig_sem >= 37 and peso_g >= 2500:
        return "Termo/peso adequado presumido, se AIG", riscos
    if ig_sem < 37 and peso_g >= 2500:
        return "Pré-termo com peso ≥ 2.500 g", riscos
    if peso_g < 1500:
        return "Muito baixo peso ao nascer", riscos
    if peso_g < 2500:
        return "Baixo peso ao nascer", riscos
    return "Classificação dependente da curva peso/IG", riscos
