...
def render_mapa_vacinal_cards(idade_meses_float: float):
    ...
    vacinas_id = {v["id"]: v for v in vacinas}
    linhas = [
        ("Até 12 meses", [
            ["bcg", "hepb_nasc", "penta_1", "penta_2", "penta_3", "rota_1", "rota_2"],
            ["pneumo10_1", "pneumo10_2", "vip_1", "vip_2", "vip_3", "menc_1", "menc_2"],
            ["febre_amarela_1", "triplice_viral_1", "covid_1", "covid_2", "covid_3", "influenza", None],
        ]),
        ("A partir de 12 meses", [
            ["pneumo10_ref", "menc_ref", "dtp_ref1", "dtp_ref2", "vop_ref1", "vop_ref2", "tetraviral"],
            ["varicela", "febre_amarela_ref", "hepa", "hpv_1", "hpv_2", "pneumo23_indigena", None],
        ]),
    ]
    atrasadas, proximas = [], []
    ...
    st.caption("As vacinas já esperadas para a idade aparecem em cor normal; as futuras aparecem sombreadas. Clique em 'Abrir' para orientações técnicas e EAPV.")
    for grupo, rows in linhas:
        for row_idx, row in enumerate(rows):
            cols = st.columns([.32, 1, 1, 1, 1, 1, 1, 1], gap="small")
            with cols[0]:
                st.markdown(f"<div class='vaccine-row-label'>{grupo if row_idx == 0 else ''}</div>", unsafe_allow_html=True)
            for i, vid in enumerate(row):
                with cols[i + 1]:
                    if vid is None:
                        st.markdown("<div class='vaccine-empty-cell'></div>", unsafe_allow_html=True)
                        continue
                    vacina = vacinas_id[vid]
                    key = f"card_vac_{vid}"
