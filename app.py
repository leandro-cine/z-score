...
def selecionar_opcoes(label, opcoes, key, default=None, cols=2, help=None, compacto=True):
    """Seleção sem multiselect/search para não abrir teclado no mobile.

    Para não poluir a tela, o padrão agora é um botão/popover compacto:
    a lista de checkboxes só aparece quando o usuário abre o seletor.
    """
    default = default or []
    if not compacto:
        return _render_checkboxes_inline(label, opcoes, key, default=default, cols=cols, help=help)

    atuais = [x for x in opcoes if st.session_state.get(f"{key}_{opcoes.index(x)}", x in default)]
    resumo = ", ".join([titulo_opcao(x) for x in atuais[:3]])
    if len(atuais) > 3:
        resumo += f" +{len(atuais)-3}"
    if not resumo:
        resumo = "Nenhum selecionado"

    st.markdown(f"<div class='compact-select-summary'><b>{label}</b><br><span>{resumo}</span></div>", unsafe_allow_html=True)
    with popover_ou_expander(f"Selecionar — {label}", expanded=False):
        return _render_checkboxes_inline("Opções", opcoes, key, default=default, cols=cols, help=help)
...
with tabs[0]:
    ...
    with st.expander("Cabeça e pescoço", expanded=False):
        ...
        with popover_ou_expander("📍 Localizações anatômicas dos linfonodos", expanded=False):
            st.caption("Use o mapa anatômico para orientar a seleção das cadeias palpáveis; depois marque as regiões abaixo.")
            render_linfo_3d()
        ...
        with popover_ou_expander("🦷 Mapa dental estruturado", expanded=False):
            st.caption("Mapa visual de apoio. Marque abaixo os dentes e alterações observadas.")
