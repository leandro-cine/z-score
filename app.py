...
try:
    from ia_prescricao import gerar_passagem_caso_ia, gerar_passagem_caso_local
except Exception:
    gerar_passagem_caso_ia = None
    gerar_passagem_caso_local = None
...
def css(sexo: str):
    ...
    .passagem-box {{ white-space:pre-wrap; background:rgba(15,23,42,.05); border:1px solid var(--border); padding:1rem; border-radius:18px; color:var(--text); font-family:inherit; line-height:1.55; }}
    .protocol-result {{ border-left:6px solid var(--accent); }}
...
def montar_dados_passagem():
    return {
        "crianca": {
            "sexo": globals().get("sexo", "—"),
            "data_nascimento": globals().get("data_nasc").strftime("%d/%m/%Y") if globals().get("data_nasc") else "—",
            "data_consulta": globals().get("data_aval").strftime("%d/%m/%Y") if globals().get("data_aval") else "—",
            "idade_cronologica": f"{globals().get('anos','—')}a {globals().get('meses','—')}m {globals().get('dias','—')}d",
            "idade_corrigida": f"{globals().get('anos_c','—')}a {globals().get('meses_c','—')}m {globals().get('dias_c','—')}d",
        },
        "nascimento": {
            "ig": f"{globals().get('idade_gest_semanas','—')}s {globals().get('idade_gest_dias','—')}d",
            "peso_nascimento": f"{globals().get('peso_nasc_g','—')} g",
            "classificacao": globals().get("class_nasc", "—"),
            "riscos": globals().get("riscos_nasc", []),
        },
        "medidas": {
            "peso": f"{globals().get('peso','—')} kg",
            "estatura": f"{globals().get('estatura','—')} cm",
            "pc": f"{globals().get('pc','—')} cm",
            "imc": f"{globals().get('imc','—'):.1f}" if isinstance(globals().get("imc"), (int, float)) else "—",
        },
        ...
        "pontos_atencao": st.session_state.get("passagem_pontos_atencao", []),
    }
...
def abrir_popup_passagem():
    dados = montar_dados_passagem()
    usar_ia = st.session_state.get("usar_ia_passagem", False)
    api_key = _segredo_openai()
    if usar_ia and gerar_passagem_caso_ia:
        texto = gerar_passagem_caso_ia(dados, api_key=api_key)
    elif gerar_passagem_caso_local:
        texto = gerar_passagem_caso_local(dados)
    else:
        texto = "Não foi possível carregar o gerador de passagem de caso. Verifique o arquivo ia_prescricao.py."
    try:
        @st.dialog("🧾 Passagem de caso")
        def _dialog_passagem():
            st.markdown("<div class='passagem-box'>" + _escape_html(texto) + "</div>", unsafe_allow_html=True)
            st.download_button("Baixar passagem em .txt", data=texto, file_name="passagem_de_caso_puericultura.txt", mime="text/plain")
            st.caption("Modelo de apoio. Revise antes de usar no prontuário ou entregar a outro profissional.")
        _dialog_passagem()
    except Exception:
        with st.expander("🧾 Passagem de caso", expanded=True):
            st.markdown("<div class='passagem-box'>" + _escape_html(texto) + "</div>", unsafe_allow_html=True)
            st.download_button("Baixar passagem em .txt", data=texto, file_name="passagem_de_caso_puericultura.txt", mime="text/plain")


def render_campo_protocolo(campo: dict, prefixo: str):
    cid = campo["id"]
    label = campo.get("label", cid)
    key = f"{prefixo}_{cid}"
    tipo = campo.get("tipo", "text")
    if tipo == "checkbox":
        return st.checkbox(label, value=campo.get("default", False), key=key)
    if tipo == "select":
        opcoes = campo.get("opcoes", [])
        default = campo.get("default", opcoes[0] if opcoes else None)
        idx = opcoes.index(default) if default in opcoes else 0
        return st.selectbox(label, opcoes, index=idx, key=key)
    if tipo == "multiselect":
        return st.multiselect(label, campo.get("opcoes", []), default=campo.get("default", []), key=key)
    if tipo == "number":
        return st.number_input(label, min_value=campo.get("min", 0), max_value=campo.get("max", 999), value=campo.get("default", 0), step=campo.get("step", 1), key=key)
    if tipo == "textarea":
        return st.text_area(label, value=campo.get("default", ""), key=key)
    return st.text_input(label, value=campo.get("default", ""), key=key)
...
with st.container():
    ...
    with ac1:
        st.session_state["usar_ia_passagem"] = st.toggle("Usar IA na passagem", value=False, help="Opcional: se houver OPENAI_API_KEY em st.secrets, a IA redige a passagem; caso contrário, o app usa modelo local.")
    with ac2:
        if st.button("🧾 Gerar passagem de caso", use_container_width=True):
            abrir_popup_passagem()
    with ac3:
        st.caption("A passagem usa os dados preenchidos no app e o resultado das seções já avaliadas.")
...
with tabs[1]:
    ...
    st.session_state["passagem_desenvolvimento"] = {"resumo": f"{clas_dev}. {texto_dev}", "fatores": fatores_dev}
...
with tabs[2]:
    ...
    with c2:
        ...
        st.session_state["passagem_vacinas"] = {"atrasadas": "; ".join(todas_atrasadas) if todas_atrasadas else "nenhuma pendência pelo cálculo atual", "proximas": "; ".join(todas_proximas[:10]) if todas_proximas else "sem próximas doses imediatas"}
...
with tabs[3]:
    ...
    st.session_state["passagem_suplementacao"] = {
        "resumo": f"Ferro: {rec['protocolo']} — {rec['dose_mg_dia']:.1f} mg/dia. Vitamina A: {vit_a['dose']} ({'indicada' if vit_a['indicada'] else 'conferir critérios'}). Vitamina D: {vit_d['dose']}."
    }
...
with tabs[5]:
    ...
    st.caption("Módulo separado para doenças comuns do ambulatório. Para ampliar protocolos extensos, edite apenas `protocolos_ambulatoriais.py`.")

    if obter_protocolos_ambulatoriais is None or campos_do_protocolo is None or executar_protocolo is None:
        st.error("Não consegui carregar `protocolos_ambulatoriais.py`. Verifique se o arquivo está na mesma pasta do app.py.")
    else:
        protocolos = obter_protocolos_ambulatoriais()
        nome_protocolo = st.selectbox("Protocolo", list(protocolos.keys()))
        meta = protocolos[nome_protocolo]
        st.markdown(f"<div class='soft-card'><div class='soft-title'>{nome_protocolo}</div><p><b>Categoria:</b> {meta.get('categoria','—')}<br><b>Objetivo:</b> {meta.get('objetivo','—')}</p></div>", unsafe_allow_html=True)
        ...
        for i, campo in enumerate(campos):
            with cols[i % 2]:
                respostas[campo["id"]] = render_campo_protocolo(campo, f"amb_{nome_protocolo}")

        dados_crianca = {
            "sexo": sexo,
            "idade_meses": idade_meses_float,
            "idade_meses_cron": idade_meses_cron,
            "peso": peso,
            "estatura": estatura,
            "pc": pc,
            "imc": imc,
            "prematuro": prematuro,
            "peso_nascimento_g": peso_nasc_g,
            "idade_gestacional_semanas": idade_gest_sem,
        }

        if st.button("Gerar análise do protocolo", type="primary", use_container_width=True):
            resultado = executar_protocolo(nome_protocolo, respostas, dados_crianca)
            st.session_state["resultado_ambulatorio"] = resultado
            st.session_state["passagem_ambulatorio"] = {
                "protocolo": nome_protocolo,
                "classificacao": resultado.get("classificacao", "—"),
                "conduta_resumo": "; ".join(resultado.get("conduta", [])[:3]),
            }

        resultado = st.session_state.get("resultado_ambulatorio")
        if resultado:
            st.markdown(f"""
            <div class='soft-card protocol-result'>
              <div class='soft-title'>Síntese clínica</div>
              <p><b>Classificação:</b> {resultado.get('classificacao','—')}<br>
              <b>Gravidade/atenção:</b> {resultado.get('gravidade','—')}</p>
              <p><b>Conduta:</b></p>{html_lista(resultado.get('conduta', []))}
              <p><b>Modelo de prescrição/orientação:</b></p>
              <div class='prescricao'>{resultado.get('prescricao','')}</div>
              <p><b>Orientações ao responsável:</b></p>{html_lista(resultado.get('orientacoes', []))}
            </div>
            """, unsafe_allow_html=True)

            if st.button("🧾 Gerar passagem de caso com este protocolo", use_container_width=True):
                abrir_popup_passagem()
